...
usage () {
  ...
  echo " -S Sort contigs in the order from largest to smallest. Usage: ./sequence-stats -S FASTA"
  echo " -h Print usage and exit"
  echo " -v Print version and exit"
  echo ""
  echo "Version 1.2 (2025)"
  echo "Author: Raymond Kiu r.k.o.kiu@bham.ac.uk"
  echo "";
}

version (){
echo "sequence-stats 1.2"
}
...
gawk 'BEGIN{RS=">";ORS=""}NR>1{n=index($0,"\n");h=substr($0,1,n-1);s=substr($0,n+1);t=s;gsub(/\n/,"",t);rec[NR-1]=">"
h"\n"s;len[NR-1]=length(t)}END{m=asorti(len,idx,"@val_num_desc");for(i=1;i<=m;i++)print rec[idx[i]]}' $FILE
...
# Call options
while getopts ':aqtcdnbresfShv' opt;do
  case $opt in
    a) fasta; exit;;
    q) fastq; exit;;
    t) fastq2fasta; exit;;
